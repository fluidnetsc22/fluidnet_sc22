...
if remote:
    ...
    if local_remote:
        if not os.path.isdir('/data/ubuntu/loaded'):
            ...
            os.system('s5cmd cp s3://bucket/3d_1gb/* /data/ubuntu/loaded/')
        ...
        print(f'Loading dataset from s3 card!')
        url_te = "pipe:aws s3 cp s3://bucket/3D_data_reduced/data_te_{00000..20479}.tar -"
        url_tr = "pipe:aws s3 cp s3://bucket/3D_data_reduced/data_tr_{00000..20479}.tar -"
        if not shuffle_training:
            tr = (wds.WebDataset(url_tr).decode(
                "pil").to_tuple('input.pyd', 'output.pyd'))
        else:
