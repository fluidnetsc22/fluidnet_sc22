...
class MultiScaleNet(nn.Module):
    ...
    def forward(self, x):
        ...
        final_out = torch.cat(
            (self.final(convN_1out), F.interpolate(
                convN_2out, (x.size()[
                    2:]), mode='bilinear', align_corners=align), F.interpolate(
                F.interpolate(
                    convN_4out, (half_size), mode='bilinear', align_corners=align), (x.size()[
                        2:]), mode='bilinear', align_corners=align)), dim=1)
