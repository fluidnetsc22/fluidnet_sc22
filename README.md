...
The training process can ba parallized both using Pytorch Data Parallel [(DP)](https://pytorch.org/docs/stable/generated/torch.nn.DataParallel.html) or Pytorch Distributed Data Parallel [(DDP)](https://pytorch.org/docs/stable/generated/torch.nn.parallel.DistributedDataParallel.html#torch.nn.parallel.DistributedDataParallel). To use more than one GPU during training the ```paralel```option in the yaml file (line 60) needs to be set to true. By default this module uses DP due to its simple implementation and because the trainings were executed in a single node. However, DDP has also been tried, and the difference between both trainings were only around 5% in training time. To launch a DDP training launch the ```fluid_net_train_ddp.py``` file instead of the classical ```fluid_net_train.py``:
