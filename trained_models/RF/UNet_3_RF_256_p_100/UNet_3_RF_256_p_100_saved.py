...
from lib import fluid, MultiScaleNet, UNet, UNet3D, FlexiUNet
...
class FluidNet(nn.Module):
    ...
    def __init__(self, mconf, it, folder, dropout=True):
        ...
        if mconf['model_name'] == 'FlexiUNet':
            scales = mconf['scales']
            pad_method = mconf['pad_method']
            kernel_sizes = mconf['kernel_sizes']
            input_res = mconf['input_res']
            self.multiScale = FlexiUNet( scales, kernel_sizes, input_res,
                    pad_method, upsample_mode='nearest')
            print(self.multiScale.global_prop())

        else:
            self.multiScale = UNet3D(self.inDims)
    ...
    def forward(self, input_, it,folder):
        ...
        if self.is3D:
            flags = input_[:,4].unsqueeze(1).contiguous()
        else:
        ...
        if (self.mconf['inputChannels']['UDiv'] or self.mconf['inputChannels']['div'] \
            ...
            if self.mconf['inputChannels']['div']:
                div = fluid.velocityDivergence(UDiv, flags).contiguous()
        ...
        if self.mconf['model'] == 'ScaleNet':
            ...
            # Initialize time variable as 1D tensor so that DataParallel can work
